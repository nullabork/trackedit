...
const REVIEW: Record<string, Record<string, [Verdict, string]>> = {
  ...
  item: {
    ...
    Scale: ["todo", "not applied when drawing; 1 on every item seen so far"],
    PackDesc: ["todo", "item skin (sign image): not drawn"], ForegroundPackDesc: ["todo", "item skin foreground: not drawn"],
