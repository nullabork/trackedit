...
function compareWithEditor(map: string, work: string): { checked: number; wrong: Map<string, number>; examples: string[] } {
...
try {
  for (const map of maps) {
    ...
    const bad = r.outOfRange > 0 || r.notExtracted > 0 || editor.wrong.size > 0;
    if (bad) failed++;
