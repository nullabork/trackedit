...
export function decide(room: RoomState, seenUid: string | null, openMapUid: string | null): TrackDecision {
  ...
  if (!room.currentMapUid && !room.nadeoHosted) {
    return { load: null, seenUid, nextPollMs: IDLE_POLL_MS, note: `${room.name} runs on a player's own server: Nadeo does not say which map it is playing (${players}).` };
  }
  if (!room.running || !room.currentMapUid) {
    return { load: null, seenUid, nextPollMs: IDLE_POLL_MS, note: `${room.name} is off — a room's server stops when it is empty. Waiting for it to start.` };
  }
  const uid = room.currentMapUid;
  if (uid === seenUid) return { load: null, seenUid, nextPollMs: POLL_MS, note: `${room.name} is still on the same map (${players}).` };
  if (seenUid === null && uid === openMapUid) return { load: null, seenUid: uid, nextPollMs: POLL_MS, note: `${room.name} is playing the open map (${players}).` };
  return { load: uid, seenUid: uid, nextPollMs: POLL_MS, note: `${room.name} ${seenUid === null ? "is playing" : "moved to"} another map (${players}).` };
}
